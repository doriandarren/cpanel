    ...
    if ($datos) {
        ...
        <div class="col-md-12 table-responsive w-margen-50">                    
            <table class="table" id="mi_tabla">
                <thead>
                    <tr>
<th>#</th><th>ID</th><th>NOMBRE</th><th>DESCRIPCION</th><th>FECHA INICIO</th><th>FECHA FIN</th><th>PORCENTAJE AVANCE</th><th>PROYECTOS ESTATUS ID</th><th>PROYECTOS DEFINICIONES ID</th>
                ...
                <tbody>
                    ...
                    foreach ($datos as $row) {
                        ?>
                        <tr>
<td><?= $i ?></td><td><?= $row->id?></td><td><?= $row->nombre?></td><td><?= $row->descripcion?></td><td><?= $row->fecha_inicio?></td><td><?= $row->fecha_fin?></td><td><?= $row->porcentaje_avance?></td><td><?= $row->proyectos_estatus_id?></td><td><?= $row->proyectos_definiciones_id?></td><td>
